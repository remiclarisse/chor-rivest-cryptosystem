...
Le rapport est organisé comme suit: en section~\ref{sec:presInria}, nous présentons Inria, ainsi que l'équipe-projet Grace, et faisons un bref descriptif des attentes du stage. Les sections~\ref{sec:cryptosysteme} et~\ref{sec:cryptanalyse} exposent les travaux déjà effectués sur le cryptosysème de Chor-Rivest et la cryptanalyse de Vaudenay. La section~\ref{sec:DLPJoux} donne divers resultats sur le calcul de logarithmes discrets dans des corps finis et aborde le papier de Joux, dont nous déduisons, en section~\ref{sec:consequences}, des améliorations possibles pour le cryptosystème de Chor-Rivest.
...
L'équipe est composée de 10 membres. Outre Daniel Augot, le responsable, il y a quatre membres permanents: Alain Couvreur, Françoise Levy-dit-Vehel, François Morain et Benjamin Smith. Les cinq autres membres sont Luca De Feo, qui est chercheur délégué, Nicholas Coxon et William George, qui effectuent leur post-doctorat, et Élise Barelli et Julien Lavauzelle, qui sont en dernière année de thèse.
...
%% Ce polynôme P ne joue aucun rôle dans le système, ni dans les attaques.
%% Je me demande si c'est la peine de lui donner un nom, voire d'en parler tout court.
...
Nous avons fini de fabriquer la clé privée ! Reste à construire la clé publique\footnote{À toutes fins utiles, la méthode utilisée pour le calcul des logarithmes discret
%% Par Sage? Quelle version?
est Pohlig-Hellman \cite{pohligHellman1978} avec du pas-de-bébé--pas-de-géant.} :
\begin{verbatim}
...
Nous présentons ici la cryptanalyse de Serge Vaudenay \cite{vaudenay2000}, publiée en 2000. Elle s'appuie sur la friabilité de $h$, i.e. sur l'existence de nombreux sous-corps de $\gf{p^h}$. Cette attaque ne requiert pas d'émission de chiffrés, elle se base seulement sur la connaissance de la clé publique. Elle exploite une faiblesse du cryptosystème, à savoir que par conception du cryptosystème, il existe plusieurs clés privées qui produisent la même clé publique.
%% Je ne suis pas convaincu que ceci soit une faiblesse. Il faut bien sûr tenir cela
%% en compte pour mener l'attaque, mais techniquement c'est juste une caractéristique
%% du système.
...
Dans le cryptosystème de Chor-Rivest, nous choisissons la clé privée de façon aléatoire puis, à partir de celle-ci, nous calculons la clé publique. Le système repose sur la difficulté de trouver une clé secrète à partir de la clé publique.
%% Tout système repose sur cette difficulté
Cependant, nous remarquons qu'il y a plusieurs clés privées \textit{équivalentes}, à savoir qu'il existe plusieurs jeux de clés privées qui donnent la même clé publique.
...
%% Notation dangereuse: on peut comprendre Q_π = Q = Q_σ
\begin{align*}

...
Serge Vaudenay fabrique un polynôme bien particulier, qui lui sert de levier pour casser le cryptosystème. Le voici :
...
$g_{p^r}$ est la norme de $g$ considéré dans l'extension de corps $\gf(p^r) \subseteq \gf(p^h)$, ainsi nous avons $(g_{p^r})^{p^r} = g_{p^r}$ et $g_{p^r}$ est générateur de $\gf(p^r)^\times$ car $g$ est primitif dans $\gf(p^h)$. Nous remarquons que $Q\left(x^{p^r}\right) = Q(x)^{p^r}$, ce qui prouve que $Q(x) \in \gf(p^r)[x]$. En effet :
\begin{align*}
...
&= Q\left(x^{p^r}\right), \\
\end{align*}
car $t^{p^{r(h/r)}} = t^{p^h} = t = t^{p^0}$. Cela montre au passage que $Q(-t) = 0$. Nous avons aussi que $Q(x)$ est de degré $h/r$. Et enfin, pour $0\leqslant j \leqslant p-1$, calculons $(g_{p^r})^{c_j}$ :
\begin{align*}
...
Comme $h/r$ est assez petit, il est peu probable qu'il existe d'autres solutions $(g_{p^r}, Q)$, et $g_{p^r}$ est donc essentiellement unique. Soulignons la particularité de ce polynôme $Q$, il est à coefficients dans $\gf(p^r)$, il est de degré $h/r$ et nous connaissons ses valeurs sur tout $\gf(p)$ : il peut donc être interpolé sur un petit ensemble de $\gf(p)$.

\paragraph*{Notation :} Pour $r$ divisant $h$, notons $$g_{p^r} := g^{(p^h-1)/(p^r-1)} = g^{1+p^r+p^{2r}+p^{3r}+\cdots+p^{h-r}},$$
la norme de $g$ considéré dans l'extension de corps $\gf(p^r) \subseteq \gf(p^h)$.

\subsection*{Attaque sachant $g_{p^r}$ et $\sigma$}

Supposons connus la norme $g_{p^r}$ et la permutation $\sigma$. Nous pouvons alors interpoler le polynôme $Q(x)$ de la proposition~\ref{premierePropVaudenay}, avec $h/r +1$ pairs ${(\alpha_{\sigma(i)}, (g_{p^r})^{c_i})}_i$. Cela donne un polynôme de degré $h/r$ dont les racines sont les conjugués de $-t$.  Par la symétrie dans la clé privée, nous pouvons sélectionner n'importe quelle racine de $Q(x)$. 
Cet algorithme fait l'objet de la figure Algorithme~\ref{algoPremiereAttaqueVaudenay}.

Nous résolvons l'attaque en calculant $g$ et $d$ grâce à l'attaque de Oded Goldreich exposée dans \cite{chorRivest1988}, une version simplifiée sera utilisée plus loin, c'est pour cela qu'elle est volontairement omise ici.

\begin{algorithm}[h]
\caption{Algorithme implémentant l'attaque sachant $g_{p^r}$ et $\sigma$}
...
L'interpolation du polynôme $Q(x)$ de la proposition~\ref{premierePropVaudenay} se fait un ensemble quelconque de $h/r+1$ éléments de $\gf(p)$. À savoir, on choisit des indices $i_0, i_1, \dots, i_{h/r}$ pris entre $0$ et $p-1$, deux à deux distincts ; et on interpole le polynôme en les $\alpha_{i_j}$ $0 \leqslant j \leqslant h/r$. 

Pour simplifier le propos, et surtout pour rendre plus lisible l'écriture, nous faisons le choix ici de prendre $i_j = j$, pour tout $0 \leqslant j \leqslant h/r$. Vaudenay ne cède pas à cette facilité.

\subsection*{Attaque sachant $g_{p^r}$}

Supposons connu l'élément $g_{p^r}$. Par la proposition~\ref{premierePropVaudenay}, nous pouvons interpoler le polynôme $Q(x)$ en les $\alpha_{\sigma(j)}$, pour $0 \leqslant j \leqslant h/r$, grâce au polynôme interpolateur de Lagrange :
\begin{align*}
...
&= \sum_{j=0}^{h/r} (g_{p^r})^{c_{j}} \prod_{\substack{0\leqslant k\leqslant h/r \\ k \neq j}}\frac{x-\alpha_{\sigma(k)}}{\alpha_{\sigma(j)}-\alpha_{\sigma(k)}}, \\
\end{align*} 
ce qui mène aux égalités : quel que soit $i$ tel que $0\leqslant i \leqslant p-1$,
$$(\dagger) \qquad (g_{p^r})^{c_i} = \sum_{j=0}^{h/r} (g_{p^r})^{c_{j}}\prod_{\substack{0\leqslant k\leqslant h/r \\ k \neq j}} \frac{\alpha_{\sigma(i)}-\alpha_{\sigma(k)}}{\alpha_{\sigma(j)}-\alpha_{\sigma(k)}}.$$

En fait, nous pouvons même écrire : quel que soit $i$ tel que $0\leqslant i \leqslant p-1$,
$$(\ddagger) \qquad (g_{p^r})^{c_i} - (g_{p^r})^{c_{0}} = \sum_{j=1}^{h/r} \left((g_{p^r})^{c_{j}} - (g_{p^r})^{c_{0}} \right)\prod_{\substack{0\leqslant k\leqslant h/r \\ k \neq j}} \frac{\alpha_{\sigma(i)}-\alpha_{\sigma(k)}}{\alpha_{\sigma(j)}-\alpha_{\sigma(k)}}.$$
En effet, soit $i$ tel que $0 \leqslant i \leqslant {h/r}$ :
$$(g_{p^r})^{c_i} - (g_{p^r})^{c_{0}} = \sum_{j=1}^{h/r} \left((g_{p^r})^{c_{j}} - (g_{p^r})^{c_{0}} \right)\prod_{\substack{0\leqslant k\leqslant h/r \\ k \neq j}} \frac{\alpha_{\sigma(i)}-\alpha_{\sigma(k)}}{\alpha_{\sigma(j)}-\alpha_{\sigma(k)}},$$
...
$$Q(x) - (g_{p^r})^{c_{0}} = \sum_{j=1}^{h/r} \left((g_{p^r})^{c_{j}} - (g_{p^r})^{c_{0}} \right)\prod_{\substack{0\leqslant k\leqslant h/r \\ k \neq j}} \frac{x-\alpha_{\sigma(k)}}{\alpha_{\sigma(j)}-\alpha_{\sigma(k)}},$$
...
\`A cause de la symétrie de la clé privée, nous pouvons choisir arbitrairement $\sigma(0)$ et $\sigma(1)$. Un algorithme naïf pour trouver la permutation $\sigma$ est de chercher exhaustivement les valeurs $\sigma(j)$ pour $2\leqslant j \leqslant h/r$, jusqu'à ce que $(\dagger)$ donne une permutation consistante.
...
\FORALL{$\pi(2), \pi(3), \dots, \pi({h/r})$ distincts deux à deux} \label{algoNaifgprBoucle}
	...
	\FORALL{$\ell \not\in S$} \label{algoNaifgprBoucle2}
		\STATE{calculer le membre de droite de $(\dagger)$ avec $\alpha_\ell$ au lieu de $\alpha_{\pi(i)}$ : $$\mathbf{res} \gets \sum_{j=0}^{h/r} (g_{p^r})^{c_{j}}\prod_{\substack{0\leqslant k\leqslant h/r \\ k \neq j}} \frac{\alpha_\ell-\alpha_{\pi(k)}}{\alpha_{\pi(j)}-\alpha_{\pi(k)}} $$}
		\IF{il existe $i$ tel que $\mathbf{res} = (g_{p^r})^{c_i}$ et $\pi(i)$ n'est pas définie}
			\STATE{$\pi(i) \gets \ell$}
...
$$\alpha_{\pi(i)} = u \alpha_{\sigma(i)} + v,$$
où la permutation $\sigma$ est celle de la clé secrète d'origine, détenue par le destinataire légitime. En exprimant le polynôme $Q(x)$ faisant intervenir la permutation $\pi$ par rapport au polynôme $Q(x)$ faisant intervenir la permutation $\sigma$, nous obtenons :
\begin{align*}
...
Il est alors plus simple de déterminer un $g$ pour avoir une clé privée équivalente, que d'utiliser l'attaque de Goldreich (comme indiqué plus haut).
...
\begin{algorithm}[h]
\caption{Algorithme implémentant l'attaque de Goldreich simplifiée}
...
\STATE{calculer les $b_i := \log_{g'}(t+\alpha_{\pi(i)})$}
...
\FOR{$i$ allant de $0$ à $p-1$}
	\IF{$\pgcd(C[i], p^h-1) = 1$}
		...
		\STATE{$\mathcal{C} \gets \{L\cdot C[n] \pmod{p^h-1}$ pour $n$ allant de $0$ à $p-1 \}$}
		\STATE{$\mathcal{B} \gets \{B[n] \pmod{p^h-1}$ pour $n$ allant de $0$ à $p-1 \}$}	
...
Bien sûr, si en entrée de l'algorithme~\ref{algoAttaqueGoldreichsimplifiee}, le ``$t$'' donné est $u t^{p^{jr}} -v$, il retourne le conjugué $g^{p^{jr}}$ du $g$ d'origine et l'entier $d - \log_{g^{p^{jr}}}(u)$.

Quand $r$ est suffisamment grand, il existe un meilleur algorithme. En fait, si $r \geqslant h/r$, la famille $\left((g_{p^r})^{c_{j}} - (g_{p^r})^{c_{0}} \right)_{0\leqslant j \leqslant h/r}$ est affinement indépendante. Cela signifie que les coefficients dans $(\dagger)$ sont les seuls coefficients dans $\gf(p)$ de l'écriture de $(g_{p^r})^{c_{i}} - (g_{p^r})^{c_{0}}$, pour $1\leqslant i \leqslant p-1$, comme combinaison linéaire des vecteurs : $$(g_{p^r})^{c_{1}} - (g_{p^r})^{c_{0}},\quad \dots,\quad (g_{p^r})^{c_{{h/r}}} - (g_{p^r})^{c_{0}}.$$

Notons $a_j^i$ le coefficient de $(g_{p^r})^{c_{{j}}} - (g_{p^r})^{c_{0}}$ pour $(g_{p^r})^{c_{i}} - (g_{p^r})^{c_{0}}$. Par $(\ddagger)$, nous avons :
\begin{align*}
...
\STATE{pré-calculer la matrice de changement de bases de la base ``classique'' vers la ``base'' $\left((g_{p^r})^{c_{i_j}} - (g_{p^r})^{c_{i_0}} \right)_{0\leqslant j \leqslant h/r}$}
\STATE{choisir arbitrairement $\pi(i_1)$ et $\pi(i_2)$ distincts dans $\{0, \dots, p-1\}$}
\FORALL{$u$ dans $\gf(p)$} 
	...
	\ENDFOR
	\IF{$\pi$ est consistante}
		\STATE{compléter $\pi(i_0)$}
		\RETURN{$\pi$}
	\ENDIF
\ENDFOR
\end{algorithmic}
...
\FORALL{$\zeta \in \gf(p^h)$ générateur de $\gf(p^r)^\times$} \label{algoTestgprboucle}
	\STATE{calculer l'équation du sous-espace affine $V$ engendré par $(\zeta^{c_{i_0}}, \dots, \zeta^{c_{i_{h/r}}})$}
	\FORALL{$i$ différent de $i_0, \dots, i_{h/r}$ }	
...
\begin{algorithmic}[1]
\REQUIRE $\gf(p^h)$, $(c_0,\dots, c_{p-1})$, $r$ divisant $h$, ${\{r_i\}}_{1\leqslant i\leqslant k}$ diviseurs de $r$ et $g_{p^{r_i}}$
\ENSURE l'ensemble des $g_{p^r}$ possibles
\STATE{choisir $\gamma$ un élément primitif de $\gf(p^r)$}
\FORALL{$i$ allant de $1$ à $k$}
	\STATE{résoudre l'équation $(\dagger)$ : $$x_i = \frac{\log(g_{p^{r_i}})}{1+p^{r_i}+p^{2r_i}+p^{3r_i}+\cdots+p^{r-r_i}} \pmod{p^{r_i}-1}$$}
\ENDFOR
\STATE{$\ell \gets \ppcm \left\{p^{r_1} - 1, p^{r_2}-1, \dots, p^{r_k} -1\right\}$}
\STATE{résoudre $x \equiv x_i \pmod{p^{r_i}-1}$ où $x$ est unique modulo $\ell$}
\STATE{$\beta \gets \gamma^{x}$}
\FOR{$y$ allant de $0$ à $(p^r-1)/\ell - 1$}\label{algoNaifgprFromgpriBoucle}
	\FOR{$e$ allant de $1$ à $(p-1)r/h - 1$}	
		\STATE{calculer la somme : $$\mathbf{res} \gets \sum_{i=0}^{p-1} \beta^{ec_i}\gamma^{ec_i\ell y}$$}
		\IF{$\mathbf{res} \neq 0$}
			\STATE{continuer la boucle ligne~\ref{algoNaifgprFromgpriBoucle}}
		\ENDIF
	\ENDFOR
	\STATE{ajouter $\beta\gamma^{\ell y}$ à la liste des $g_{p^r}$ possibles}
\ENDFOR
\end{algorithmic}
\end{algorithm}

Chaque boucle de la ligne~\ref{algoNaifgprFromgpriBoucle} demande en moyenne $\O(pr^2)$ opérations dans $\gf(p)$.

Nous pouvons maintenant définir l'algorithme global de l'attaque.

\begin{defi}
Soit $G$ un graphe orienté acyclique (\textit{DAG for directed acyclic graph}) étiqueté enraciné dont la racine est étiquetée par un corps fini $\gf(p^r)$ et tel que pour chaque arête $u \rightarrow v$ de $G$, l'étiquette $l(u)$ de $u$ est un sous-corps maximal (au sens de l'inclusion) de l'étiquette $L(v)$ de $v$ et une extension de $\gf(p)$. On dit que $G$ est un DAG $p$-factorisant de $\gf(p^r)$.
\end{defi}

\begin{defi}
\`A une extension $\gf(p^r)$ et $G$ son DAG $p$-factorisant, on associe :
$$C(G) := \sum_{v} \frac{\#L(v) -1}{\ppcm\{\#L(w)-1 \mid v \leftarrow w\}},$$
où nous prenons pour convention : $\ppcm(\emptyset) = 1$.
\end{defi}

Nous pouvons définir un algorithme pour calculer $g_{p^r}$ avec pour compléxité $\O(pr^2C(G))$, où $G$ est le DAG $p$-factorisant de $\gf(p^r)$. Donc, nous pouvons casser le cryptosystème de Chor-Rivest avec comme paramètre $h$ ni un nombre premier, ni un carré de nombre premier, avec une complexité de
$$\O\left(\min_{\substack{r \text{ divisant } h \\ r^2 \geqslant h}} \; \min_{\substack{G \text{ est un DAG}\\p\text{-factorisant} \\ \text{ de } \gf(p^r)}} \; 
pr^2C(G)\right)$$

\begin{algorithm}[h]
\caption{Attaque général}
\label{algoGeneral}
\begin{algorithmic}[1]
...
\FOR{$u$ sommet de $G$, tel que pour tout $u \leftarrow u_i$, $u_i$ a été visité,}
	\STATE{l'algorithme~\ref{algogprFromgpri} avec $\gf(p^r)=L(u)$ et $\gf(p^{r_i}) = L(u_i)$}
\ENDFOR
\STATE{appliquer l'algorithme~\ref{algogpr} à $g_{p^r}$}
\end{algorithmic}
\end{algorithm}

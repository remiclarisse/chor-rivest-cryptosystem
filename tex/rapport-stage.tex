...
\subsection{Attaque directe sachant $\mbf g_{p^r}$}
...
ce qui mène aux égalités :
\begin{equation}\label{eqn:interpolation}
(\mbf g_{p^r})^{c_i} = \sum_{j=0}^{h/r} (\mbf g_{p^r})^{c_{j}} \mathcal L(\alpha_{\sigma(i)}),\qquad \forall i : 0\leqslant i \leqslant p-1.
\end{equation}
...
La complexité de cet algorithme est grossièrement $\O(rp^{h/r})$ opérations dans $\gf{p}$ : la boucle à la ligne~\ref{boucle:algoNaifgprBoucle2} fait en moyenne $\O(pr/h)$ itérations, chacune avec une complexité $\O(h)$, et nous avons besoin de $\O(p^{h/r}-1)$ itérations de cette boucle.
...
\subsection{Attaque améliorée sachant $\mbf g_{p^r}$}

En fait, nous pouvons même écrire : quel que soit $i$ tel que $0\leqslant i \leqslant p-1$,
$$(\ddagger) \qquad (\mbf g_{p^r})^{c_i} - (\mbf g_{p^r})^{c_{0}} = \sum_{j=1}^{h/r} \left((\mbf g_{p^r})^{c_{j}} - (\mbf g_{p^r})^{c_{0}} \right)L(\alpha_{\sigma(i)}).$$

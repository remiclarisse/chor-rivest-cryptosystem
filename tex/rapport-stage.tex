...
L'exposition du cryptosystème en section~\ref{sec:cryptosysteme}, la cryptanalyse de Serge Vaudenay en section~\ref{sec:cryptanalyse} et l'algorithme d'Antoine Joux en section~\ref{sec:DLPJoux}, nous incite à prendre
$p$ et $h$ deux nombres premiers et tels que $h \leqslant p$ pour ``réparer'' le cryptosystème.

%Donner résultats précédents sur les tentatives (Madrid)

\paragraph{Prélude.} La confection de la clé publique du cryptosytème requière le calcul de logarithmes dans $\gf{p^h}$. Nous aimerions tirer avantage de la particularité de ces logarithmes, à savoir qu'on veut déterminer le logarithme d'éléments de la forme $t + \alpha$, où $t$ est tel que $\gf{p^h} = \gf{p}[t]$ et où $\alpha \in \gf{p}$. Essentiellement, si nous reprenons les notations de \ref{sec:cryptosysteme} et \ref{s-sec:algoJoux}, on souhaite grossièrement que $t = X \pmod{I_k(X)}$, avec $k = h$. En effet, grâce à cela, en nous ayant recourt à l'algorithme de Joux, il suffirait de ne faire que la première partie du crible: la collecte de relation entre polynômes linéaires. Malheureusement, on s’aperçoit immédiatement que ce n'est pas possible, car dans le cryptosystème $t \in \gf{p^h}$, alors que $I_k(X)$ n'est pas un polynôme à coefficient dans $\gf{p}$. Nous nous proposons d'explorer cette idée, qui, si elle porte ses fruits, permettrait de surmonter l'obstacle du calcul des logarithmes. Nous examinerons après si cela rend vulnérable le cryptosystème.

Pour fixer les idées, nous donnerons en \verb|sage| les commandes utilisées pour effectuer les différents calculs ainsi qu'un exemple à taille humaine pour les paramètres:
\begin{verbatim}
sage: p = 17
sage: k = 7 # joue le rôle du h, degré de l'extension
\end{verbatim}

\paragraph{Imbrication.} Étant donné que $h \leqslant p$, nous pouvons imbriquer $\gf{p^h}$ dans $\gf{p^{2h}}$, et satisfaire à la toute première étape de l'algorithme. Nous faisons donc une extension quadratique de $\gf{p^h}$, puis on cherche un polynôme irréductible convenable $I_h(X)\in\gf{p^2}[X]$, divisant $h_1(X)X^p - h_0(X)$, représentant l'extension de corps $\extension{\gf{p^{2h}}}{\gf{p^2}}$. Cela donne le diagramme :
\begin{figure}[h]
\begin{center}
\begin{tikzpicture}
...
\begin{verbatim}
sage: while true :
sage:     h0 = Fp2X.random_element(2)
sage:     h1 = Fp2X.random_element(1)
sage:     fa = list((h1 * X ** p - h0).factor())
...
Dans cette exécution ces lignes de code nous ont donné:
$$\gf{17^{2}} = \frac{\gf{17}[X]}{(\mu(X))}, \text{ où } \mu(X) := X^2 + 16X +3\in  \gf{17}[X],$$
et, en notant $\alpha = X \pmod{\mu(X)}$, les polynômes:
\begin{eqnarray*}
h_0(X) &=& (11\alpha + 16)X^2 + (6\alpha + 16)X + \alpha+ 10, \\
h_1(X) &=& (16\alpha + 13)X + 10\alpha+7, \\
I_h(X) &=& X^7 + (11\alpha + 6)X^6 + (3\alpha + 7)X^5 + (11\alpha + 8)X^4  \\&& + (5\alpha + 8)X^3 +(4\alpha + 8)X^2 + (9\alpha + 5)X + 11\alpha + 16. \\
\end{eqnarray*}
...
\paragraph{Relations.}On souhaite avoir des relations seulement entre les polynômes linéaires unitaires, sans les coefficients de $\gf{p^2}$. Pour cela, nous allons fixer un élément primitif $g \in \gf{p^{2h}}$, où: $$\gf{p^{2h}} = \frac{\gf{p^2}[X]}{(I_h(X))}.$$
La représentation en \verb|sage| donne:
\begin{verbatim}
sage: Fp2k.<b> = Fp2X.quotient_ring(Ik)
\end{verbatim}
...
sage:     return recurcive_multorder (x, card - 1, prime_factors (card - 1))

sage: def recurcive_multorder (x, order, prime_fa) :
sage:     A = x.parent()
sage:     for p in prime_fa :
sage:         if x ** (order / p) == A.one() :
sage:             return recurcive_multorder (x, order / p, 
